...
/** "Ma'lumotlar" page: one row per vehicle trip, stage columns fill as the
 *  vehicle passes each checkpoint (karyer kirish/chiqish, zavod kirish/chiqish). */
export function Trips() {
  const { t } = useTranslation();
  return (
    <div className="flex flex-col gap-3.5 p-6">
      <h1 className="m-0 text-[17px] font-semibold">{t('nav_data')}</h1>
      <TripsTable />
    </div>
  );
}
